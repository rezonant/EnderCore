...
public class Scheduler {
   ...
   *          <p>
   *          Note: passing in {@link Dist#CLIENT} on a dedicated server will work, but your task will never be called. Please avoid doing this to save
   *          processing.
   */
  ...
  public void onClientTick(TickEvent.ClientTickEvent event) {
    if (event.phase == TickEvent.Phase.END) {
      ...
      if (!serverQueue.isEmpty() && !Minecraft.getInstance().hasSingleplayerServer()) {
        serverQueue.clear();
      }
    }
  }
